...
func (l *WithdrawalListLogic) WithdrawalList(req *types.WithdrawalListReq) (resp *types.WithdrawalListResp, err error) {
    ...
    SELECT drr.address AS address, COALESCE(drr.amount+drr.fee,0) AS amount, e.epoch AS epoch, UNIX_TIMESTAMP(drr.create_block_time) AS create_at, UNIX_TIMESTAMP(drr.claim_at) AS claim_at, drr.claimed AS claimed 
