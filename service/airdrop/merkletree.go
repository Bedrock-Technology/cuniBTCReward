...
func GetAirdrop(chainId uint, contract string, epoch uint64, leaves []TreeLeaf, shares []string) (*[]*model.AirDropRecord, []byte, error) {
	if len(leaves) == 0 {
		return nil, nil, fmt.Errorf("leaves is empty")
	}
	if len(leaves) != len(shares) {
		return nil, nil, fmt.Errorf("leaves length does not match shares length")
	}
	...
	if len(proof.Proofs) != len(leaves) {
		return nil, nil, fmt.Errorf("proof length does not match leaves length")
	}
	// AirDropRecord
	airdropRecords, err := lo.MapErr(proof.Proofs, func(proofData *smt.StandardMerkleLeafProofData, k int) (*model.AirDropRecord, error) {
		values := proofData.Value
	...
	return &airdropRecords, tree.GetRoot(), nil
}
